...
function RightSeller(props) {
  ...
  const logoutHandler = () => {
      axios.get(`${SELLER_SERVER}/logout`).then(response => {
  ...
  if (seller.sellerData && !seller.sellerData.isAuth) {
    return (
    ...
    return (
      <Menu mode={props.mode}>
        ...
        <Menu.Item key="myuploads">
          <a href="/history" style={{fontWeight: "bolder"}}>My Uploads</a>
        </Menu.Item>
...
export default withRouter(RightSeller);
